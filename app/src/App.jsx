...
export default function App() {
  const { yearlyStats, topicStats, loading } = useGoldData()
        ...
        >
          {LEGEND.map(({ color, label, note }) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div
                style={{ width: 12, height: 12, borderRadius: 2, background: color, flexShrink: 0 }}
              />
              <span style={{ fontFamily: 'IBM Plex Sans', fontSize: 13, color: 'var(--text)' }}>
                {label}
              </span>
