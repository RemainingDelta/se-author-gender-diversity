...
function AuthorPanel({ name, venueAuthors }) {
  ...
  const collabTotal = collab.female_presenting + collab.male_presenting + collab.unclassified + collab.unknown
  ...
  return (
    <div style={{
      ...
      borderLeft: `3px solid ${color}`, borderRadius: 8, padding: '24px',
      marginTop: 20,
    }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <span style={{ fontFamily: 'Playfair Display', fontSize: 22, color: 'var(--text)' }}>{name}</span>
        <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 12, color, padding: '2px 8px', border: `1px solid ${color}`, borderRadius: 4 }}>
          {primary.gender_label}
        </span>
        {venuesFound.map(v => (
          <span key={v} style={{ fontFamily: 'IBM Plex Mono', fontSize: 11, color: VENUE_COLOR[v] }}>
            {v}
          </span>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 24 }}>
        {/* Top topics */}
        <div>
          <div style={{ fontFamily: 'IBM Plex Sans', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
            Top Topics
          </div>
          {topTopics.length ? topTopics.map(([topic, count]) => (
            <div key={topic} style={{ fontFamily: 'IBM Plex Sans', fontSize: 12, color: 'var(--text)', marginBottom: 4, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
              <span style={{ color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{topic}</span>
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 11, color: 'var(--muted)', flexShrink: 0 }}>×{count}</span>
            </div>
        ...
        <div>
          ...
          {[['First', firstCount], ['Middle', middleCount], ['Last', lastCount]].map(([label, count]) => (
            <div key={label} style={{ marginBottom: 6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                <span style={{ fontFamily: 'IBM Plex Sans', fontSize: 12, color: 'var(--muted)' }}>{label}</span>
                <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 11, color: 'var(--text)' }}>{count}</span>
              </div>
              <div style={{ height: 4, background: 'var(--border)', borderRadius: 2 }}>
                <div style={{ height: '100%', width: `${pct(count, allPositions.length)}%`, background: color, borderRadius: 2 }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          {[
            ...
            <div key={label} style={{ marginBottom: 6 }}>
              ...
              <div style={{ height: 4, background: 'var(--border)', borderRadius: 2 }}>
                <div style={{ height: '100%', width: `${pct(count, collabTotal)}%`, background: barColor, borderRadius: 2 }} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <section id="search" style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px' }}>
      ...
      <p style={{ fontFamily: 'IBM Plex Sans', fontSize: 14, color: 'var(--muted)', marginBottom: 32 }}>
        Look up any author across all venues to see their gender label, topics, and authorship profile.
      </p>
      ...
      <div style={{ position: 'relative', maxWidth: 520 }}>
        <input
          ...
          style={{
            ...
            fontFamily: 'IBM Plex Sans', fontSize: 14, color: 'var(--text)',
            outline: 'none',
          }}
        />
            ...
            {matches.map(name => (
              <div
                ...
                style={{
                  ...
                  fontFamily: 'IBM Plex Sans', fontSize: 13, color: 'var(--text)',
