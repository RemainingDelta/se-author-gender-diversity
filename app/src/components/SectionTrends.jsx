...
export default function SectionTrends({ yearlyStats }) {
  ...
  return (
    <section id="trends" style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px' }}>
      ...
      >
        Percentage of <span style={{ color: activeColor }}>{activeLabel}</span> authors per year
          ...
          >
            {['All Authors', 'First Author'].map((label, i) => {
              ...
              return (
                <button
                  key={label}
                  onClick={() => setFirstAuthorOnly(i === 1)}
                  style={{
                    fontFamily: 'IBM Plex Sans',
                    fontSize: 12,
                    padding: '5px 14px',
            ...
            <div key={`${venue}_m`} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div
                style={{
                  width: 32,
                  height: 4,
                  background: `repeating-linear-gradient(90deg, ${VENUE_COLOR[venue]} 0 6px, transparent 6px 9px)`,
                  flexShrink: 0,
                }}
              />
              <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 13, color: 'var(--text)' }}>
