      ...
      {payload
        ...
        .map((p) => {
          ...
          return (
            <div
              key={p.dataKey}
              style={{ color: VENUE_COLOR[venue], opacity: isMale ? 0.65 : 1, marginBottom: 2 }}
            >
              {venue}
              {gender === 'all' ? (isMale ? ' ♂' : ' ♀') : ''}: {p.value.toFixed(1)}%
            </div>
          )
        })}
    </div>
    ...
    <section id="venues" style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px' }}>
      ...
      <div
        style={{
          fontFamily: 'IBM Plex Mono',
          fontSize: 11,
          color: gender === 'male' ? 'var(--male)' : gender === 'all' ? '#a78bfa' : 'var(--female)',
          marginBottom: 6,
        }}
      >
        %{' '}
