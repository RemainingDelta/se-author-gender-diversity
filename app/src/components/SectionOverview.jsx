...
const VENUE_COLOR = { ICSE: 'var(--icse)', ECSA: 'var(--ecsa)', MSR: 'var(--msr)', ICSME: 'var(--icsme)' }
const VENUE_FULL  = {
  ...
  return (
    <div style={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden', gap: 1 }}>
      {segments.map(({ key, value, color }) => (
        <div key={key} style={{ flex: value / total, background: color, minWidth: value > 0 ? 2 : 0 }} />
      ))}
    </div>
  )
}
...
function VenueCard({ venue, totals }) {
  ...
  return (
    <div style={{
      ...
      borderTop: `3px solid ${color}`, borderRadius: 8, padding: '24px',
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 4 }}>
        <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 13, fontWeight: 600, color }}>{venue}</span>
      </div>
      <div style={{ fontFamily: 'IBM Plex Sans', fontSize: 12, color: 'var(--muted)', marginBottom: 20 }}>
        {VENUE_FULL[venue]}
      </div>
